  ...
  return (
    <header className="flex items-center justify-between px-4 md:px-8 py-4 bg-gradient-to-b from-primary-200 to-transparent border-b border-slate-800 sticky top-0 z-50 backdrop-blur-xl">
      <div className="flex items-center gap-3 md:gap-4">
        ...
        >
          <img src="/favicon.svg" alt="Smart City Logo" className="w-full h-full" />
        </motion.div>
      ...
      <div className="hidden md:flex gap-8">
        <div className="flex flex-col items-center">
          <span className="text-2xl font-bold font-mono text-slate-100">{devices.length}</span>
          <span className="text-[10px] text-slate-500 uppercase tracking-wider">Dispositivos</span>
        </div>
        <div className="flex flex-col items-center">
          <span className="text-2xl font-bold font-mono text-accent-green">{onlineDevices}</span>
      ...
      <div className="flex items-center gap-3">
        <motion.button 
          ...
          title={connection.isConnected ? 'Clique para desconectar' : 'Clique para conectar'}
        >
          {connection.isConnected ? (
            <>
