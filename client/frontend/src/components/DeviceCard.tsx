...
const deviceColorClasses: Record<
  DeviceType,
...
const trafficLightColors: Record<
  string,
...
export function DeviceCard({ device, index }: DeviceCardProps) {
  ...
  const checkAlert = () => {
    ...
    if (device.type === "temperature_sensor") {
      ...
      return (
        device.sensorData.value < minTemp || device.sensorData.value > maxTemp
      );
    }
  ...
  const trafficLight = trafficLightState
    ? trafficLightColors[trafficLightState] || trafficLightColors.red
    : null;

  const brightness =
    device.type === "street_lamp"
  ...
  return (
    <motion.div
      className={`
        ...
        ${
          isSelected
            ? "ring-2 ring-accent-cyan ring-offset-2 ring-offset-primary"
            : ""
        }
        ${device.status === "offline" ? "opacity-60" : ""}
        ${
          hasAlert
            ? "ring-2 ring-accent-red ring-offset-1 ring-offset-primary animate-pulse"
            : ""
        }
      `}
      ...
      <div className="mb-4">
        ...
        {device.type === "traffic_light" && trafficLight && (
          <div className="mt-3 flex items-center gap-3">
            <div
              className={`w-8 h-8 rounded-full ${trafficLight.bg} ${trafficLight.glow}`}
            />
          ...
          <div className="mt-3 flex items-baseline gap-1">
            <span
              className={`text-3xl font-bold font-mono ${
                hasAlert ? "text-accent-red" : colors.text
              }`}
            >
            ...
            {hasAlert && (
              <span className="ml-2 text-xs text-accent-red font-medium">
                ALERTA!
              </span>
            )}
          </div>
